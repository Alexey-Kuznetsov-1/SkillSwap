...
const SkillPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [skillsRes, usersRes, imagesRes] = await Promise.all([
          fetch('/db/skills.json'),
          fetch('/db/users.json'),
          fetch('/db/skill-images.json')
        ]);
        ...
        const images = imagesData.data as SkillImage[];
        ...
        if (skill) {
          // Загружаем изображения для текущего навыка
          const skillImageObjects = images.filter(img => img.skillId === skill.id);
          const imageUrls = skillImageObjects.map(img => img.images);
          ...
          const similar = skills
            ...
            .map(s => {
              ...
              return {
                ...
                direction: s.type,
                skills: [
                  { id: s.id, name: s.title, direction: s.type, subcategory: null }
                ]
              };
            });
  ...
  const userSkills = allSkills.filter(s => s.author === currentSkill.author && s.type === 'teach');
  const canTeach = userSkills.map(s => s.title);
  
  const totalPages = Math.ceil(similarSkills.length / itemsPerPage);
  const currentSkills = similarSkills.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
              ...
              <div className={styles.skillCard}>
                ...
                <div className={styles.galleryWrapper}>
                  ...
                  <div className={styles.descriptionWrapper}>
                    <div>
                      <h1 className={styles.skillTitle}>{currentSkill.title}</h1>
                      <span className={styles.skillCategory}>
                        {categoryLabels[currentSkill.category] || currentSkill.category} / {subCategoryLabels[currentSkill.subCategory] || currentSkill.subCategory}
                      </span>
                      <p className={styles.skillDescription}>{currentSkill.description}</p>
                    </div>
                    <button className={styles.exchangeButton} onClick={handleOfferExchange}>
                      Предложить обмен
                    </button>
                  </div>

                  {/* Галерея — справа, используем компонент ImageView */}
                  <div className={styles.galleryContainer}>
                    <ImageView imagesSkill={skillImages} />
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className={styles.carouselContainer}>
                <div className={styles.carousel}>
                  {currentSkills.map((skill, idx) => {
                    const isLastCard = idx === currentSkills.length - 1 && currentPage < totalPages - 1;
                    return (
